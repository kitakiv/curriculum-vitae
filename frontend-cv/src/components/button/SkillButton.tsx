...
export default function SkillButton({ children, click, tailwind, active = false }: { children: React.ReactNode, click?: () => void, tailwind?: string, active?: boolean }) {
    ...
    return (
        <button
            onClick={click}
            className={ `${active ? colorRound :  `hover:${colorRound}` } ${tailwind}  flex justify-center items-center w-auto h-auto px-8 py-3 rounded-full bg-opacity-80 ease-in-out shadow-bg100 shadow-lg transition-all  duration-700 hover:scale-105`}>
