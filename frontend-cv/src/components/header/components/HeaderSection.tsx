...
export default function HeaderSection({children}: {children: React.ReactNode}) {
    ...
    return (
        <div className="fixed transition duration-700 z-50 w-full top-0 flex items-center bg-transparent justify-between  lg:p-3 p-2 hover:bg100 gap-4" onMouseEnter={() => showItems(true)} onMouseLeave={() => showItems(false)} style={{backgroundColor: show ? "var(--color-bg-33)" : "transparent"}}>
            {children}
        </div>
    )
}