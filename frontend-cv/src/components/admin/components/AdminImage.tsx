...
export default function AdminImage({ tailwind, path, alt }: { tailwind?: string, path: string, alt?: string }) {
    ...
    return (
        <AdminBorder >
        <FlexibleForm tailwind="padding-admin flex justify-center items-center w-full" type="oneElement" initialValues={{ mainImage: "/" }} formInputs={form.mainImageForm.inputs} submitFunction={alertMessage} schema={schema.mainImage} >
